...
export default class App extends Component {
  constructor() {
    ...
    this.getRatingMovie = () => {
      this.SwapiService.getGuestSessionRatedMovie(this.state.sessionId, this.state.ratingMovieCurrentPage)
        .then((data) => {
          this.setState({
    ...
    this.searchMovieRatedList = () => {
      this.SwapiService.getGuestSessionRatedMovie(this.state.sessionId, this.state.ratingMovieCurrentPage).then(
        (body) => {
          this.setState({
    ...
    this.searchMovieList = (state = this.state) => {
      this.SwapiService.getResource(
        ...
        .then((body) => {
          const moviesWithRating = this.refreshMoviesRatingSub(body.results, this.state.moviesDataRatingArr)
          this.setState({
      ...
      this.overwriteMoviesDataWithNewRating = (id, rating) => {
        let moviesDataRatingArr = { ...this.state.moviesDataRatingArr }
        moviesDataRatingArr[id] = rating
        this.setState({
          moviesDataRatingArr: moviesDataRatingArr,
        })
      }
      //----------------------------------------------------
  ...
  refreshMoviesRatingSub(moviesData_, moviesDataRatingArr_)
  {
    const refreshMuviesData = moviesData_.map((movie) => {
      movie.rating = moviesDataRatingArr_[movie.id] ? moviesDataRatingArr_[movie.id] : 0
      return movie
    })
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.ratingMovieCurrentPage !== this.state.ratingMovieCurrentPage) {
      this.searchMovieRatedList()
    }
    if (prevState.activeHeaderBtn !== this.state.activeHeaderBtn) {
      if (this.state.activeHeaderBtn == 'search') {
        this.setState({
          searchMovie: 'return',
        })
      }
  ...
  render() {
    const {
      activeHeaderBtn, moviesData, ratingMoviesCounter,
      moviesCounter,
    ...
    const searchBox = <GenresProvider value={this.state.moviesGenres}>
                            <Search
    ...
    const ratedBox = <GenresProvider value={this.state.moviesGenres}>
                        <Rated
                          getRatingMovie={this.getRatingMovie}
    ...
    return (
      <div className="container">
        <Header checkHeaderBtnActive={this.checkHeaderBtnActive} 
                refreshMovieRating={this.refreshMovieRating} 
                test ='test'/>
        {search}
        {errorMessage}
